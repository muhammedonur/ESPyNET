...
with tf.Graph().as_default(), tf.Session() as sess:
    ...
    output_l0, output_l1, output_l2, output_l3, output_l4, output_l5 = \
        PyNET(training_phone_, instance_norm=True, instance_norm_level_1=False)
    ...
    dslr_flat = tf.reshape(training_dslr_, [-1, TARGET_SIZE])
    ...
    dslr_vgg = vgg.net(vgg_dir, vgg.preprocess(training_dslr_ * 255))
    ...
    logs = open("../models/PyNET/logs.txt", "w+")
    logs.close()
    ...
    for i in range(num_train_iters + 1):
        ...
        if i % eval_step == 0:
            ...
            for j in range(num_test_batches):
                ...
                if LEVEL < 2:
                    losses = sess.run([loss_generator, loss_content, loss_mse, loss_psnr, loss_ms_ssim], \
                                    feed_dict={training_phone_: phone_images, training_dslr_: dslr_images})
                else:
            ...
            logs = open("../models/PyNET/logs.txt", "a")
            logs.write(logs_gen)
            logs.write('\n')
            logs.close()
            ...
            enhanced_crops = sess.run(enhanced, feed_dict={training_phone_: visual_test_crops, training_dslr_: dslr_images})
